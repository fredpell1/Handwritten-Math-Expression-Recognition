...
class Decoder(torch.nn.Module):
    def __init__(
        self, input_size, hidden_size, output_size, num_features, batch_size
    ) -> None:
        super().__init__()
    ...
    def forward(self, input, hidden, encoder_output):
        ...
        #context vector
        c = torch.sum(alphas * encoder_output, 0, keepdim=True)
        new_h = torch.cat((hn, c), -1)
        new_h = torch.tanh(self.Wc(new_h))
