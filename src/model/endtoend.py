...
class HME2LaTeX(torch.nn.Module):
    ...
    def __init__(self, cnn, encoder, decoder, seq_size, batch_size, language_size, EOS_TOKEN, SOS_token, max_sequence_size)-> None:
        super().__init__()
        self.cnn = cnn
    ...
    def forward(self,images,labels):
        ...
        init_hidden = torch.cat((h[0], h[1]), -1).unsqueeze(0)
        init_c = torch.cat((c[0], c[1]), -1).unsqueeze(0)
        probs = torch.zeros((self.seq_size,self.batch_size,self.language_size))
        #training time
        ...
        #testing time
            ...
            SOS_tensor = torch.tensor(self.SOS_token, dtype=torch.float32).expand((1, self.batch_size, 1))
